async function createReel(payload) {
  ...
  if (!response.ok) {
    throw new Error(data.detail || "Errore durante l'avvio della generazione.");
  }
...
async function createStoryboard(payload) {
  const response = await fetch("/api/reels/storyboard", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    throw new Error(data.detail || "Errore durante la creazione dello storyboard.");
  }
...
async function renderStoryboard(payload) {
  const response = await fetch("/api/reels/render-storyboard", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    ...
    if (response.status === 404 || response.status === 405) {
      ...
      return createReel({
        ...
        duration_seconds: Math.max(
          8,
          Math.round(scenes.reduce((sum, scene) => sum + (scene.duration_seconds || 3), 0))
        ),
...
async function getReelStatus(jobId) {
  ...
  if (!response.ok) {
    throw new Error(data.detail || "Errore durante il controllo del Reel.");
  }
...
async function listMediaAssets() {
  ...
  if (!response.ok) {
    throw new Error(data.detail || "Errore durante il caricamento della media library.");
  }
...
async function uploadMediaAsset(file) {
  ...
  if (!response.ok) {
    throw new Error(data.detail || "Errore durante l'upload dell'immagine.");
  }
...
async function regenerateScene(payload) {
  const response = await fetch("/api/reels/regenerate-scene", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
  ...
  if (!response.ok) {
    throw new Error(data.detail || "Errore durante la rigenerazione della scena.");
  }
